...
function resolveMathTypeServerUrl(configuredUrl?: string): string {
  const serverUrl = String(configuredUrl || MATHTYPE_SERVER_URL || '').trim().replace(/\/$/, '');
  if (!serverUrl) {
    throw new Error(
      'File Word có MathType OLE nhưng chưa cấu hình VITE_MATHTYPE_SERVER_URL.',
    );
  }
  ...
  if (typeof window !== 'undefined') {
    ...
    const backendHost = (() => {
      try { return new URL(serverUrl).hostname.toLowerCase(); } catch { return ''; }
    })();
    const isLocalBackend = backendHost === 'localhost' || backendHost === '127.0.0.1';
    if (!isLocalPage && isLocalBackend) {
      throw new Error(
        'Đang chạy production nhưng VITE_MATHTYPE_SERVER_URL vẫn trỏ về localhost:8000. ' +
        'Hãy đặt URL backend MathType production trên Vercel rồi Redeploy.',
      );
    }
  }
  return serverUrl;
...
export function getMathTypeServerUrl(): string {
  return resolveMathTypeServerUrl();
}
...
async function convertOleToLatex(
  ...
  try {
    const resolvedServerUrl = resolveMathTypeServerUrl(serverUrl);
    console.log(`⏳ Connecting to MathType server: ${resolvedServerUrl}`);
    const alive = await wakeUpServer(resolvedServerUrl, 90_000);
    if (!alive) throw new Error(`Không kết nối được ${resolvedServerUrl}/health`);

    const res = await fetch(`${resolvedServerUrl}/v1/convert`, {
      method: 'POST',
    ...
    if (!res.ok) {
      ...
      throw new Error(`Server MathType trả HTTP ${res.status}${detail ? `: ${detail.slice(0, 300)}` : ''}`);
    }
    const data = await res.json() as {
      results?: Array<{ id?: string; latex?: string; error?: string }>;
    };
    ...
    console.warn(`⚠️ MathType server (${serverUrl}) unavailable:`, e);
    throw new Error(
      e instanceof Error
        ? `Không chuyển được MathType OLE: ${e.message}`
        : 'Không chuyển được MathType OLE do backend không phản hồi.',
    );
  }
  return ridToLatex;
}
...
// ============================================================
// ESSAY-ONLY WORD IMPORT (QUESTION FILE + ANSWER FILE)
// ============================================================
export interface EssaySolutionImportItem {
  number: number;
  solution: string;
  images: ImageData[];
}

export interface EssaySolutionImport {
  sourceFileName: string;
  items: Record<number, EssaySolutionImportItem>;
  formulaCount: number;
  reusableSolutionCount: number;
}

interface EssayWordBlock {
  number: number;
...
const ESSAY_QUESTION_RE = /^\s*(?:C(?:âu|au)|Bài|Bai)\s*(\d+)\s*[.\-:)]*\s*(.*)$/i;
const ESSAY_NUMBERED_RE = /^\s*(\d+)\s*[.\-:)]\s+(.+)$/;
const ESSAY_SOLUTION_RE = /^\s*(?:L(?:ời|oi)\s*gi(?:ải|ai)|Đáp\s*án|Dap\s*an|Hướng\s*dẫn\s*giải|Huong\s*dan\s*giai)\s*[:.\-]?\s*(.*)$/i;
const ESSAY_HEADING_RE = /^\s*(?:PHẦN|PHAN|ĐỀ|DE|ĐÁP\s*ÁN|DAP\s*AN|HƯỚNG\s*DẪN|HUONG\s*DAN)\b/i;

async function readWordForEssay(
  file: File,
  config?: { mathTypeServerUrl?: string }
): Promise<{ paragraphs: ParagraphData[]; images: ImageData[]; formulaCount: number }> {
  const serverUrl = config?.mathTypeServerUrl ?? MATHTYPE_SERVER_URL;
  const arrayBuffer = await file.arrayBuffer();
  const zip = await JSZip.loadAsync(arrayBuffer);
  const { images, imageRelMap } = await extractImages(zip);
  const oleItems = await extractOleItems(zip);
  const oleLatexMap = oleItems.length ? await convertOleToLatex(oleItems, serverUrl) : new Map<string, string>();
  if (oleItems.length > 0 && oleLatexMap.size !== oleItems.length) {
    const missingIds = oleItems.filter((item) => !oleLatexMap.has(item.id)).map((item) => item.id);
    throw new Error(
      `Backend MathType chỉ chuyển được ${oleLatexMap.size}/${oleItems.length} công thức. ` +
      `Công thức lỗi: ${missingIds.join(', ')}. Dừng nhập file để tránh mất công thức trong đề/đáp án.`,
    );
  }
  const documentXml = await zip.file('word/document.xml')?.async('string');
  if (!documentXml) throw new Error('Không tìm thấy document.xml trong file Word');
...
function paragraphImages(paragraph: ParagraphData, images: ImageData[]): ImageData[] {
  const found: ImageData[] = [];
  paragraph.imageRIds.forEach((rId) => {
    const image = images.find((item) => item.rId === rId);
    if (image && !found.some((item) => item.id === image.id)) found.push(image);
  });
  return found;
}

function appendUniqueImages(target: ImageData[], incoming: ImageData[]) {
  incoming.forEach((image) => {
    if (!target.some((item) => item.id === image.id)) target.push(image);
  });
}
...
function startMatch(text: string): { number: number; rest: string } | null {
  const named = text.match(ESSAY_QUESTION_RE);
  if (named) return { number: Number(named[1]), rest: String(named[2] || '').trim() };
  const numbered = text.match(ESSAY_NUMBERED_RE);
  if (numbered) return { number: Number(numbered[1]), rest: String(numbered[2] || '').trim() };
  return null;
}
...
function parseEssayBlocks(paragraphs: ParagraphData[], images: ImageData[]): EssayWordBlock[] {
  ...
  const flush = () => {
    if (!current) return;
    const hasText = current.beforeSolution.some(Boolean) || current.afterSolution.some(Boolean);
    const hasImages = current.questionImages.length > 0 || current.solutionImages.length > 0;
    if (hasText || hasImages) blocks.push(current);
    current = null;
  };
  ...
  paragraphs.forEach((paragraph) => {
    ...
    if (match) {
      ...
      current = {
        number: match.number,
        beforeSolution: match.rest ? [match.rest] : [],
        afterSolution: [],
        questionImages: [],
        solutionImages: [],
        inSolution: false,
      };
      appendUniqueImages(current.questionImages, paragraphImages(paragraph, images));
      return;
    }

    if (!text && paragraph.imageRIds.length === 0) return;
    if (!current && hasExplicitQuestions) return;
    ...
    if (solutionMarker) {
      ...
      appendUniqueImages(block.solutionImages, paragraphImages(paragraph, images));
      return;
    }
...
/**
 * Đọc riêng file đề tự luận. Không dùng Gemini/OCR; MathType OLE được chuyển
 * sang LaTeX đúng một lần bằng server chuyển đổi MathType hiện có.
 */
export const parseEssayQuestionsFromWord = async (
  file: File,
  config?: { mathTypeServerUrl?: string }
): Promise<ExamData> => {
  const { paragraphs, images } = await readWordForEssay(file, config);
  const blocks = parseEssayBlocks(paragraphs, images);
  if (blocks.length === 0) throw new Error('Không tìm thấy câu tự luận trong file đề.');

  const questions: Question[] = blocks.map((block, index) => ({
    number: 400 + (Number(block.number) || index + 1),
    text: toEssayHtml(block.beforeSolution),
    type: 'writing',
    options: [],
    correctAnswer: null,
    solution: block.afterSolution.length ? toEssayHtml(block.afterSolution) : '',
    solutionImages: block.solutionImages,
    images: block.questionImages,
    part: 'PHẦN 4',
    section: { letter: '4', name: 'Tự luận', points: '' },
  }));
  ...
  return {
    ...
    sections: [{
      name: 'PHẦN TỰ LUẬN',
      description: 'Trình bày lời giải chi tiết',
      sectionType: 'writing',
      questions,
    }],
    images,
...
/** Đọc riêng file đáp án và trả lời giải đã chuyển MathType/OLE sang LaTeX. */
export const parseEssaySolutionsFromWord = async (
  file: File,
  config?: { mathTypeServerUrl?: string }
): Promise<EssaySolutionImport> => {
  const { paragraphs, images, formulaCount } = await readWordForEssay(file, config);
  const blocks = parseEssayBlocks(paragraphs, images);
  if (blocks.length === 0) throw new Error('Không tìm thấy lời giải trong file đáp án.');

  const items: Record<number, EssaySolutionImportItem> = {};
  blocks.forEach((block, index) => {
    const localNumber = Number(block.number) || index + 1;
    // Có marker “Lời giải/Đáp án” thì chỉ lấy phần sau marker. Nếu file đáp án
    // chỉ ghi “Câu 1. ...lời giải...” thì dùng toàn bộ nội dung sau số câu.
    const solutionLines = block.afterSolution.length > 0 ? block.afterSolution : block.beforeSolution;
    items[localNumber] = {
      number: localNumber,
      solution: toEssayHtml(solutionLines),
      images: block.solutionImages.length > 0 ? block.solutionImages : block.questionImages,
    };
  });

  const reusableSolutionCount = Object.values(items).filter((item) =>
    item.solution.replace(/<br\s*\/?\s*>/gi, '').replace(/<[^>]+>/g, '').trim().length > 0,
  ).length;
  if (reusableSolutionCount === 0) {
    throw new Error(
      'File đáp án chỉ có ảnh hoặc không có nội dung chữ/MathType đọc được. ' +
      'Hãy dùng văn bản, Equation/OMML hoặc MathType OLE để lưu lời giải thành LaTeX.',
    );
  }

  return { sourceFileName: file.name, items, formulaCount, reusableSolutionCount };
};

export function mergeEssaySolutions(exam: ExamData, imported: EssaySolutionImport): ExamData {
  const mergedQuestions = exam.questions.map((question, index) => {
    const localNumber = Number(question.number) >= 400 ? Number(question.number) - 400 : index + 1;
    const item = imported.items[localNumber];
    if (!item) return question;
    return {
      ...question,
      solution: item.solution || question.solution || '',
      solutionImages: item.images || [],
    };
  });
  const byNumber = new Map(mergedQuestions.map((question) => [question.number, question]));
  return {
    ...exam,
    questions: mergedQuestions,
    sections: exam.sections.map((section) => ({
      ...section,
      questions: section.questions.map((question) => byNumber.get(question.number) || question),
    })),
  };
}
